...
open_dbshell() {
  open_terminal "LocalAdvisor DB Shell" "$(pwd)/$BACKEND_DIR/dbshell.sh"
}
...
show_menu() {
  ...
  echo " 7) Database: Spring Shell (opens interactive DB shell in new terminal)"
  echo " 8) Resync password   (update postgres pwd from stack_my_settings)"
  echo " 9) Exit"
  echo "=============================="
...
show_help() {
  echo "LocalAdvisor Stack Manager"
  echo ""
  echo "Usage: $0 {menu|start|stop|logs|psql|dbshell|-h}"
  echo ""
  echo "Commands:"
  ...
  echo "  dbshell   Open Spring Shell interactive DB shell (new terminal)"
